...
void pubTF(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    ...
    const Quaterniond q_cam{estimator.ric[0]};
    transform_cam.transform.rotation.x = q_cam.x();
    transform_cam.transform.rotation.y = q_cam.y();
    transform_cam.transform.rotation.z = q_cam.z();
    transform_cam.transform.rotation.w = q_cam.w();
