  ...
  double & correction_yaw_rad)
{
  ...
  for (auto & keyframe : keyframes_) {
    if (keyframe.cloud && !keyframe.cloud->empty()) {
      pcl::transformPointCloud(*keyframe.cloud, *keyframe.cloud, delta);
    }
  ...
  const rclcpp::Time & scan_stamp)
{
  ...
  const tf2::Transform current_pose = to_tf2_transform(tf_target);
  ...
  auto keyframe_cloud = std::make_shared<pcl::PointCloud<pcl::PointXYZ>>(*scan_cloud);
  keyframes_.push_back(Keyframe{scan_stamp, keyframe_cloud, current_pose});
  last_keyframe_pose_ = current_pose;
  last_keyframe_stamp_ = scan_stamp;
  integrateGlobalCloud(keyframe_cloud);
...
void VerticalLidarMapper::scanCallback(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan_msg)
{
  ...
  if (exclude_floor_points_) {
    const std::size_t points_before = scan_cloud->size();
    auto & points = scan_cloud->points;
    const float z_cut = static_cast<float>(floor_z_max_);
    points.erase(
      std::remove_if(
        points.begin(),
        points.end(),
        [z_cut](const pcl::PointXYZ & p) {
          return std::isfinite(p.z) && p.z <= z_cut;
        }),
      points.end());
    if (points.size() != points_before) {
      floor_points_dropped_ += (points_before - points.size());
      scan_cloud->width = static_cast<uint32_t>(points.size());
      scan_cloud->height = 1;
  ...
  if (drop_global) {
    ...
    if (integration_mode_ == "continuous") {
      ...
      (void)maybeIntegrateAsKeyframe(scan_cloud, tf_target, scan_stamp);
    }
  }
