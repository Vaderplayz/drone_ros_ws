...
SESSION="${SESSION:-obs_avoid_mission_clone_stack}"
USE_TMUX="${USE_TMUX:-0}"
...
ENABLE_MISSION_OBS_AVOID="${ENABLE_MISSION_OBS_AVOID:-1}"
MISSION_OBS_AVOID_LAUNCH_FILE="${MISSION_OBS_AVOID_LAUNCH_FILE:-mission_obs_avoid_clone.launch.py}"
...
if [[ "${ENABLE_MISSION_OBS_AVOID}" == "1" ]]; then
  ...
  LAWNMOWER_RUN_CMD="ros2 run obs_avoid lawnmower_mapping_mode --ros-args -p use_sim_time:=true ${CMD_VEL_REMAP_ARG}"
  MISSION_OBS_AVOID_CMD="set +u; source '${ROS_SETUP}' || true; set -u; if ! ros2 pkg prefix mission_obs_avoid >/dev/null 2>&1; then echo '[error] mission_obs_avoid package not found in overlay'; echo '[hint] COLCON_LOG_PATH=/tmp/colcon_log_mission colcon build --base-paths ${ROS_WS}/src/obs_avoid/mission_obs_avoid --packages-select mission_obs_avoid'; exec bash; fi && ros2 launch mission_obs_avoid ${MISSION_OBS_AVOID_LAUNCH_FILE} use_sim_time:=true"
else
  ...
  LAWNMOWER_RUN_CMD="ros2 run obs_avoid lawnmower_mapping_mode --ros-args -p use_sim_time:=true"
...
kill_existing_stack() {
  ...
  kill_if_running "ros2 run obs_avoid lawnmower_mapping_mode"
  ...
  kill_if_running "ros2 launch mission_obs_avoid mission_obs_avoid_clone.launch.py"
  kill_if_running "ros2 run mission_obs_avoid mission_interceptor_node"
  kill_if_running "ros2 run mission_obs_avoid mission_local_planner_mode_a_clone"
  kill_if_running "mission_local_planner_mode_a"
...
write_terminator_layout_json() {
  ...
  cmd_mavros="set +u; source '${ROS_SETUP}' || true; set -u; echo '[wait] waiting for mavros package ...' && until ros2 pkg prefix mavros >/dev/null 2>&1; do sleep 1; done && while true; do ros2 launch mavros '${MAVROS_LAUNCH_FILE}' fcu_url:='${FCU_URL}' respawn_mavros:='${MAVROS_RESPAWN}' use_sim_time:=true; rc=\$?; if [[ \$rc -eq 130 ]]; then echo '[info] mavros stopped by user'; break; fi; echo \"[warn] mavros exited (rc=\$rc), retrying in 2s\"; sleep 2; done"
  cmd_bridge_clock="set +u; source '${ROS_SETUP}' || true; set -u; ros2 run ros_gz_bridge parameter_bridge '/clock@rosgraph_msgs/msg/Clock[gz.msgs.Clock'"
  cmd_bridge_vert="set +u; source '${ROS_SETUP}' || true; set -u; ros2 run ros_gz_bridge parameter_bridge '${TOPIC_VERT}@sensor_msgs/msg/LaserScan[gz.msgs.LaserScan' --ros-args -p use_sim_time:=true"
  cmd_bridge_horiz="set +u; source '${ROS_SETUP}' || true; set -u; ros2 run ros_gz_bridge parameter_bridge '${TOPIC_HORIZ}@sensor_msgs/msg/LaserScan[gz.msgs.LaserScan' --ros-args -p use_sim_time:=true"
  cmd_scan_override="set +u; source '${ROS_SETUP}' || true; set -u; ros2 run vertical_lidar_mapper scan_frame_override_node --ros-args -p use_sim_time:=true"
  cmd_tf_pair="set +u; source '${ROS_SETUP}' || true; set -u; ros2 run tf2_ros static_transform_publisher --x 0 --y 0 --z 0 --yaw 0 --pitch -1.570796 --roll 0 --frame-id base_link --child-frame-id lidar_vert_link --ros-args -p use_sim_time:=true & ros2 run tf2_ros static_transform_publisher --x 0 --y 0 --z 0 --yaw 0 --pitch 0 --roll 0 --frame-id base_link --child-frame-id lidar_horiz_link --ros-args -p use_sim_time:=true"
  cmd_odom_bridge="set +u; source '${ROS_SETUP}' || true; set -u; echo '[wait] waiting for MAVROS nodes ...' && until ros2 node list 2>/dev/null | grep -q '^/mavros'; do sleep 1; done && if ! ros2 topic list 2>/dev/null | grep -q '^/mavros/local_position/odom$'; then echo '[warn] /mavros/local_position/odom not listed yet; px4_odom_flatten_node will wait for messages.'; fi && echo '[run] starting px4_odom_flatten_node first (odom->base_link)' && ros2 run odom_flatten px4_odom_flatten_node --ros-args -p use_sim_time:=true -p odom_topic:=/mavros/local_position/odom -p parent_frame:=odom -p child_frame:=base_link"
  cmd_mavros_params="set +u; source '${ROS_SETUP}' || true; set -u; echo '[wait] waiting for px4_odom_flatten_node ...' && until ros2 node list 2>/dev/null | grep -q '^/px4_odom_flatten_node$'; do sleep 1; done && echo '[run] applying MAVROS use_sim_time params ...' && for n in \$(ros2 node list | grep '^/mavros'); do ros2 param set \"\$n\" use_sim_time true 2>/dev/null || true; done && echo '[done] mavros params applied'; exec bash"
  cmd_user_mode="set +u; source '${ROS_SETUP}' || true; set -u; echo '[ready] mode pane is blank. choose one:' && echo '  ${USER_CTRL_RUN_CMD}' && echo '  ${SPIRAL_RUN_CMD}' && echo '  ${LAWNMOWER_RUN_CMD}' && exec bash"
  cmd_mission_obs_avoid="${MISSION_OBS_AVOID_CMD}"
  cmd_dwa="echo '[info] external obs_avoid local_planner_mode_a disabled in mission-clone stack (planner runs inside mission_obs_avoid_clone launch)'; exec bash"
  cmd_rviz="set +u; source '${ROS_SETUP}' || true; set -u; rviz2 --ros-args -p use_sim_time:=true"
...
# Preflight: ensure overlay + mission clone launch are available before opening panes.
set +u
if ! source "${ROS_SETUP}"; then
  echo "[warn] failed to source ${ROS_SETUP}; trying fallback overlay sourcing." >&2
  source /opt/ros/rolling/setup.bash || true
  if [[ -f "${ROS_WS}/install/local_setup.bash" ]]; then
    source "${ROS_WS}/install/local_setup.bash" || true
  fi
fi
set -u
...
if [[ "${ENABLE_MISSION_OBS_AVOID}" == "1" ]]; then
  if ! ros2 pkg prefix mission_obs_avoid >/dev/null 2>&1; then
    echo "[error] mission_obs_avoid package not found in current overlay." >&2
    echo "[hint] build with:" >&2
    echo "       cd ${ROS_WS}" >&2
    echo "       colcon build --base-paths src/obs_avoid/mission_obs_avoid --packages-select mission_obs_avoid" >&2
    exit 1
  fi

  MISSION_PREFIX="$(ros2 pkg prefix mission_obs_avoid 2>/dev/null || true)"
  MISSION_LAUNCH_PATH="${MISSION_PREFIX}/share/mission_obs_avoid/launch/${MISSION_OBS_AVOID_LAUNCH_FILE}"
  if [[ ! -f "${MISSION_LAUNCH_PATH}" ]]; then
    echo "[error] launch file not installed: ${MISSION_LAUNCH_PATH}" >&2
    echo "[hint] rebuild + source overlay:" >&2
    echo "       cd ${ROS_WS}" >&2
    echo "       colcon build --base-paths src/obs_avoid/mission_obs_avoid --packages-select mission_obs_avoid" >&2
...
if ! tmux has-session -t "${SESSION}" 2>/dev/null; then
  ...
  tmux send-keys -t "${local_sim_mavros}" \
    "set +u; source '${ROS_SETUP}' || true; set -u; echo '[wait] waiting for mavros package ...' && until ros2 pkg prefix mavros >/dev/null 2>&1; do sleep 1; done && while true; do ros2 launch mavros '${MAVROS_LAUNCH_FILE}' fcu_url:='${FCU_URL}' respawn_mavros:='${MAVROS_RESPAWN}' use_sim_time:=true; rc=\$?; if [[ \"\$rc\" -eq 130 ]]; then echo '[info] mavros stopped by user'; break; fi; echo \"[warn] mavros exited (rc=\$rc), retrying in 2s\"; sleep 2; done; exec bash" C-m
  ...
  tmux send-keys -t "${local_br_clock}" \
    "set +u; source '${ROS_SETUP}' || true; set -u; ros2 run ros_gz_bridge parameter_bridge '/clock@rosgraph_msgs/msg/Clock[gz.msgs.Clock'" C-m

  local_br_vert="$(tmux split-window -h -d -P -F "#{pane_id}" -t "${local_br_clock}")"
  tmux send-keys -t "${local_br_vert}" \
    "set +u; source '${ROS_SETUP}' || true; set -u; ros2 run ros_gz_bridge parameter_bridge '${TOPIC_VERT}@sensor_msgs/msg/LaserScan[gz.msgs.LaserScan' --ros-args -p use_sim_time:=true" C-m

  local_br_horiz="$(tmux split-window -v -d -P -F "#{pane_id}" -t "${local_br_clock}")"
  tmux send-keys -t "${local_br_horiz}" \
    "set +u; source '${ROS_SETUP}' || true; set -u; ros2 run ros_gz_bridge parameter_bridge '${TOPIC_HORIZ}@sensor_msgs/msg/LaserScan[gz.msgs.LaserScan' --ros-args -p use_sim_time:=true" C-m
  ...
  tmux send-keys -t "${local_nodes_mavros_params}" \
    "set +u; source '${ROS_SETUP}' || true; set -u; echo '[wait] waiting for px4_odom_flatten_node ...' && until ros2 node list 2>/dev/null | grep -q '^/px4_odom_flatten_node$'; do sleep 1; done && echo '[run] applying MAVROS use_sim_time params ...' && for n in \$(ros2 node list | grep '^/mavros'); do ros2 param set \"\$n\" use_sim_time true 2>/dev/null || true; done && echo '[done] mavros params applied'; exec bash" C-m
  ...
  tmux send-keys -t "${local_obs_mode}" \
    "set +u; source '${ROS_SETUP}' || true; set -u; echo '[ready] mode pane is blank. choose one:' && echo '  ${USER_CTRL_RUN_CMD}' && echo '  ${SPIRAL_RUN_CMD}' && echo '  ${LAWNMOWER_RUN_CMD}' && exec bash" C-m

  local_obs_dwa="$(tmux split-window -h -d -P -F "#{pane_id}" -t "${local_obs_mode}")"
  tmux send-keys -t "${local_obs_dwa}" \
    "echo '[info] external obs_avoid local_planner_mode_a disabled in mission-clone stack (planner runs inside mission_obs_avoid_clone launch)'; exec bash" C-m
  ...
  tmux send-keys -t "${SESSION}:rviz.0" \
    "set +u; source '${ROS_SETUP}' || true; set -u; rviz2 --ros-args -p use_sim_time:=true" C-m
