...
MAPPING_3D_ENABLE_SCAN_MATCHING="${MAPPING_3D_ENABLE_SCAN_MATCHING:-true}"
...
validate_environment() {
  ...
  if [[ -z "${MAPPING_3D_TARGET_FRAME}" ]]; then
    fail "MAPPING_3D_TARGET_FRAME must not be empty"
  fi
  if [[ "${MAPPING_3D_REQUIRE_2D_MAP}" != "0" &&
  ...
  if [[ "${MAPPING_3D_ENABLE_RELATIVE_POSE_GATE}" != "true" &&
    "${MAPPING_3D_ENABLE_RELATIVE_POSE_GATE}" != "false" ]]; then
    fail "MAPPING_3D_ENABLE_RELATIVE_POSE_GATE must be true or false"
  fi
  if [[ "${MAPPING_3D_ENABLE_SCAN_MATCHING}" != "true" &&
    "${MAPPING_3D_ENABLE_SCAN_MATCHING}" != "false" ]]; then
    fail "MAPPING_3D_ENABLE_SCAN_MATCHING must be true or false"
  fi
  if [[ "${MAPPING_3D_SCAN_MATCHING_DROP_ON_FAILURE}" != "true" &&
    "${MAPPING_3D_SCAN_MATCHING_DROP_ON_FAILURE}" != "false" ]]; then
    fail "MAPPING_3D_SCAN_MATCHING_DROP_ON_FAILURE must be true or false"
  fi
}
